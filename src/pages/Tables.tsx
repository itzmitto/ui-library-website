...
const sidebarItems = [
  ...
  { label: "Tables", path: "/elements/tables", active: true },
];
...
export default function Tables() {
  ...
  const endIndex = Math.min(tables.length, endRow * columns);

  const visibleTables = tables.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: TableItem) => {
    setSelected(item);
  }, []);
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          <div className="all-header">
            <h1>Tables</h1>
            <p>Open-Source tables made with CSS or Tailwind</p>
          </div>
          <div
            ref={gridRef}
            className="all-grid"
