...
type IphoneItem = (typeof iphones)[0];

const IphoneCard = memo(
  ({
    item,
    index,
    columns,
...
IphoneCard.displayName = "IphoneCard";

export default function Iphone() {
  const [selected, setSelected] = useState<IphoneItem | null>(null);
  ...
  const totalRows = Math.ceil(iphones.length / columns);
  ...
  const endIndex = Math.min(iphones.length, endRow * columns);

  const visibleIphones = iphones.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: IphoneItem) => {
    setSelected(item);
  }, []);
    ...
    <div className="all-page">
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          ...
          >
            {visibleIphones.map((item, offset) => {
