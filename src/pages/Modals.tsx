...
const sidebarItems = [
  ...
  { label: "Modals", path: "/elements/modals", active: true },
...
type ModalItem = (typeof modals)[0];

const ModalCard = memo(
  ({
    item,
    index,
    columns,
...
export default function Modals() {
  ...
  const handleSelect = useCallback((item: ModalItem) => {
    setSelected(item);
  }, []);

  const rowHeight = CARD_HEIGHT + GRID_GAP;
  const totalRows = Math.ceil(modals.length / columns);
  ...
  const endIndex = Math.min(modals.length, endRow * columns);

  const visibleModals = modals.slice(startIndex, endIndex);
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          <div className="all-header">
            <h1>Modals</h1>
            <p>Open-Source modals made with CSS or Tailwind</p>
          </div>
