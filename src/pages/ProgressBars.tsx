...
import { progressBars } from "../data/ProgressBarsData";
import ComponentModal from "../components/ComponentModal";
import "./All.css";
import "../styling/ProgressBars.css";
...
const sidebarItems = [
  ...
  { label: "Progress Bars", path: "/elements/progress-bars", active: true },
...
type ProgressBarItem = (typeof progressBars)[0];

const ProgressBarCard = memo(
  ({
    item,
    index,
    columns,
    ...
    index: number;
    columns: number;
    onSelect: (item: ProgressBarItem) => void;
  }) => {
...
export default function ProgressBars() {
  const [selected, setSelected] = useState<ProgressBarItem | null>(null);
  ...
  const totalRows = Math.ceil(progressBars.length / columns);
  ...
  const endIndex = Math.min(progressBars.length, endRow * columns);

  const visibleProgressBars = progressBars.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: ProgressBarItem) => {
    setSelected(item);
  }, []);
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          <div className="all-header">
            <h1>Progress Bars</h1>
            <p>Open-Source progress bars made with CSS or Tailwind</p>
          </div>
          <div
            ref={gridRef}
            className="all-grid"
          ...
          >
            {visibleProgressBars.map((item, offset) => {
