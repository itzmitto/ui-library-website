...
const sidebarItems = [
  ...
  { label: "Avatars", path: "/elements/avatars", active: true },
...
type AvatarItem = (typeof avatars)[0];
...
export default function Avatars() {
  ...
  const totalRows = Math.ceil(avatars.length / columns);
  ...
  const endIndex = Math.min(avatars.length, endRow * columns);
  const visibleAvatars = avatars.slice(startIndex, endIndex);
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          <div className="all-header">
            <h1>Avatars</h1>
            <p>Open-Source avatars made with CSS or Tailwind</p>
          </div>
          <div
            ref={gridRef}
            className="all-grid"
          ...
          >
            {visibleAvatars.map((item, offset) => {
