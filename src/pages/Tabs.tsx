...
import { tabs } from "../data/componentsData";
...
const sidebarItems = [
  ...
  { label: "Tabs", path: "/elements/tabs", active: true },
...
export default function Tabs() {
  const [selected, setSelected] = useState<TabItem | null>(null);
  ...
  const totalRows = Math.ceil(tabs.length / columns);
  ...
  const endIndex = Math.min(tabs.length, endRow * columns);

  const visibleTabs = tabs.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: TabItem) => {
    setSelected(item);
  }, []);
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          <div className="all-header">
            <h1>Tabs</h1>
            <p>Open-Source tabs made with CSS or Tailwind</p>
          </div>
