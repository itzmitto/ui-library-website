...
const sidebarItems = [
  ...
  { label: "Pagination", path: "/elements/pagination", active: true },
...
type PaginationItem = (typeof pagination)[0];

const PaginationCard = memo(
  ({
    item,
    index,
    columns,
    ...
    index: number;
    columns: number;
    onSelect: (item: PaginationItem) => void;
  }) => {
...
export default function Pagination() {
  const [selected, setSelected] = useState<PaginationItem | null>(null);
  ...
  const handleSelect = useCallback((item: PaginationItem) => {
    setSelected(item);
  }, []);

  const rowHeight = CARD_HEIGHT + GRID_GAP;
  const totalRows = Math.ceil(pagination.length / columns);
  ...
  const endIndex = Math.min(pagination.length, endRow * columns);

  const visiblePagination = pagination.slice(startIndex, endIndex);
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          <div className="all-header">
            <h1>Pagination</h1>
            <p>Open-Source pagination made with CSS or Tailwind</p>
          </div>
          ...
          >
            {visiblePagination.map((item, offset) => {
