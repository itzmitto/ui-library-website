...
type ToggleItem = (typeof toggles)[0];
...
export default function Toggleswitches() {
  const [selected, setSelected] = useState<ToggleItem | null>(null);
  ...
  const totalRows = Math.ceil(toggles.length / columns);
  ...
  const endIndex = Math.min(toggles.length, endRow * columns);

  const visibleToggles = toggles.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: ToggleItem) => {
    setSelected(item);
  }, []);
    ...
    <div className="all-page">
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          ...
          >
            {visibleToggles.map((item, offset) => {
