...
type LoaderItem = (typeof loaders)[0];

const LoaderCard = memo(
  ({
    item,
    index,
    columns,
...
export default function Loaders() {
  ...
  const totalRows = Math.ceil(loaders.length / columns);
  ...
  const endIndex = Math.min(loaders.length, endRow * columns);

  const visibleLoaders = loaders.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: LoaderItem) => {
    setSelected(item);
  }, []);
    ...
    <div className="all-page">
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          ...
          >
            {visibleLoaders.map((item, offset) => {
