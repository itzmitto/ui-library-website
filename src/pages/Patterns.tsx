...
type PatternItem = (typeof patterns)[0];
...
export default function Patterns() {
  ...
  const totalRows = Math.ceil(patterns.length / columns);
  ...
  const endIndex = Math.min(patterns.length, endRow * columns);

  const visiblePatterns = patterns.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: PatternItem) => {
    setSelected(item);
  }, []);
    ...
    <div className="all-page">
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          ...
          >
            {visiblePatterns.map((item, offset) => {
