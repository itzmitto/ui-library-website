...
type BadgeItem = (typeof badges)[0];
...
export default function Badges() {
  ...
  const totalRows = Math.ceil(badges.length / columns);
  ...
  const endIndex = Math.min(badges.length, endRow * columns);

  const visibleBadges = badges.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: BadgeItem) => {
    setSelected(item);
  }, []);
