...
import { logins } from "../data/loginsData";
...
type LoginItem = (typeof logins)[0];
...
export default function Logins() {
  ...
  const handleSelect = useCallback((item: LoginItem) => {
    setSelected(item);
  }, []);

  const rowHeight = CARD_HEIGHT + GRID_GAP;
  const totalRows = Math.ceil(logins.length / columns);
  ...
  const endIndex = Math.min(logins.length, endRow * columns);

  const visibleLogins = logins.slice(startIndex, endIndex);
