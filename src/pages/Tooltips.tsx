...
type TooltipItem = (typeof tooltips)[0];
...
const TooltipCard = memo(
  ({
    ...
    index: number;
    columns: number;
    onSelect: (item: TooltipItem) => void;
  }) => {
...
export default function Tooltips() {
  const [selected, setSelected] = useState<TooltipItem | null>(null);
  ...
  const totalRows = Math.ceil(tooltips.length / columns);
  ...
  const endIndex = Math.min(tooltips.length, endRow * columns);

  const visibleTooltips = tooltips.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: TooltipItem) => {
    setSelected(item);
  }, []);
    ...
    <div className="all---page">
      ...
      <div className="all---layout">
        ...
        <main className="all---main">
          ...
          >
            {visibleTooltips.map((item, offset) => {
