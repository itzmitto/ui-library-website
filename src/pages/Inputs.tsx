...
type InputItem = (typeof inputs)[0];
...
export default function Inputs() {
  const [selected, setSelected] = useState<InputItem | null>(null);
  ...
  const totalRows = Math.ceil(inputs.length / columns);
  ...
  const endIndex = Math.min(inputs.length, endRow * columns);

  const visibleInputs = inputs.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: InputItem) => {
    setSelected(item);
  }, []);
