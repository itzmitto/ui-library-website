...
type HeroSectionItem = (typeof heroSections)[0];

const HeroSectionCard = memo(
  ({
    item,
    index,
    columns,
    ...
    item: HeroSectionItem;
    index: number;
    columns: number;
    onSelect: (item: HeroSectionItem) => void;
  }) => {
...
HeroSectionCard.displayName = "HeroSectionCard";
...
export default function HeroSections() {
  ...
  const totalRows = Math.ceil(heroSections.length / columns);
  ...
  const endIndex = Math.min(heroSections.length, endRow * columns);

  const visibleHeroSections = heroSections.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: HeroSectionItem) => {
    setSelected(item);
  }, []);
    ...
    <div className="all-page">
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          ...
          >
            {visibleHeroSections.map((item, offset) => {
