...
import { accordions } from "../data/AccordionsData";
...
const sidebarItems = [
  ...
  { label: "Accordions", path: "/elements/accordions", active: true },
...
type AccordionItem = (typeof accordions)[0];
...
export default function Accordions() {
  const [selected, setSelected] = useState<AccordionItem | null>(null);
  ...
  const totalRows = Math.ceil(accordions.length / columns);
  ...
  const endIndex = Math.min(accordions.length, endRow * columns);

  const visibleAccordions = accordions.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: AccordionItem) => {
    setSelected(item);
  }, []);
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          <div className="all-header">
            <h1>Accordions</h1>
            <p>Open-Source accordions made with CSS or Tailwind</p>
          </div>
          <div
            ref={gridRef}
            className="all-grid"
          ...
          >
            {visibleAccordions.map((item, offset) => {
