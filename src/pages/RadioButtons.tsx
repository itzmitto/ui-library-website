...
import { radiobuttons } from "../data/radioButtonsData";
...
type RadioButtonItem = (typeof radiobuttons)[0];

const RadioButtonCard = memo(
  ({
    item,
    index,
    columns,
...
export default function RadioButtons() {
  const [selected, setSelected] = useState<RadioButtonItem | null>(null);
  ...
  const totalRows = Math.ceil(radiobuttons.length / columns);
  ...
  const endIndex = Math.min(radiobuttons.length, endRow * columns);

  const visibleRadioButtons = radiobuttons.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: RadioButtonItem) => {
    setSelected(item);
  }, []);
    ...
    <div className="all-page">
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          ...
          >
            {visibleRadioButtons.map((item, offset) => {
