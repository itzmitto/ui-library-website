...
const sidebarItems = [
  ...
  { label: "Alerts", path: "/elements/alerts", active: true },
...
export default function Alerts() {
  ...
  const totalRows = Math.ceil(alerts.length / columns);
  ...
  const endIndex = Math.min(alerts.length, endRow * columns);

  const visibleAlerts = alerts.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: AlertItem) => {
    setSelected(item);
  }, []);
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          <div className="all-header">
            <h1>Alerts</h1>
            <p>Open-Source alerts made with CSS or Tailwind</p>
          </div>
