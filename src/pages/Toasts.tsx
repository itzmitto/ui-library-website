...
import { toasts } from "../data/ToastsData";
...
const sidebarItems = [
  ...
  { label: "Toasts", path: "/elements/toasts", active: true },
...
type ToastItem = (typeof toasts)[0];
...
export default function Toasts() {
  const [selected, setSelected] = useState<ToastItem | null>(null);
  ...
  const totalRows = Math.ceil(toasts.length / columns);
  ...
  const endIndex = Math.min(toasts.length, endRow * columns);

  const visibleToasts = toasts.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: ToastItem) => {
    setSelected(item);
  }, []);
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          <div className="all-header">
            <h1>Toasts</h1>
            <p>Open-Source toast notifications made with CSS or Tailwind</p>
          </div>
          <div
            ref={gridRef}
            className="all-grid"
