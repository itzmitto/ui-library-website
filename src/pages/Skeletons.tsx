...
import { skeletons } from "../data/skeletonsData";
...
type SkeletonItem = (typeof skeletons)[0];
...
const SkeletonCard = memo(
  ({
    ...
    index: number;
    columns: number;
    onSelect: (item: SkeletonItem) => void;
  }) => {
...
export default function Skeletons() {
  const [selected, setSelected] = useState<SkeletonItem | null>(null);
  ...
  const totalRows = Math.ceil(skeletons.length / columns);
  ...
  const endIndex = Math.min(skeletons.length, endRow * columns);

  const visibleSkeletons = skeletons.slice(startIndex, endIndex);

  const handleSelect = useCallback((item: SkeletonItem) => {
    setSelected(item);
  }, []);
      ...
      <div className="all-layout">
        ...
        <main className="all-main">
          <div className="all-header">
            <h1>Skeletons</h1>
            <p>Open-Source skeletons made with CSS or Tailwind</p>
          </div>
          <div
            ref={gridRef}
            className="all-grid"
          ...
          >
            {visibleSkeletons.map((item, offset) => {
